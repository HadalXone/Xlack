...
import org.xailor.xlack.internal.AnsiColor.BrightGreen
import org.xailor.xlack.internal.AnsiColor.Green
import org.xailor.xlack.internal.AnsiColor.Red
...
class SlackApiMonitor(
  ...
  private fun hookLoggingRequest(request: Request): Request {
    ...
    if (config.logRequest) {
      logger.info("${"Request".bg(BrightGreen)} [${request.method}] ${request.url}\nbody=>$bodyString")
    }
  ...
  private fun hookLoggingResponse(method: String, url: HttpUrl, response: Response, duration: Duration): Response {
    ...
    if (config.logResponse && (!config.logWhenError || ok)) {
      ...
      val bgColor = if (ok) Green else Red
